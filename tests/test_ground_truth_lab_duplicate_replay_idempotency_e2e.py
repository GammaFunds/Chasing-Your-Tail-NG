from __future__ import annotations

import hashlib
import json
from pathlib import Path
import re
import tempfile
import unittest

from ground_truth_scenario_contract import (
    GroundTruthExpectedSummaryV1,
    GroundTruthScenarioManifestV1,
    RECORD_KIND,
    SCHEMA_VERSION_V1,
)
from ground_truth_summary_builder import (
    build_completed_ground_truth_summary_v1,
)
from observation_location_link_orchestrator import (
    run_bounded_observation_location_link_correlation,
)
from observation_location_link_writer import (
    LocationLinkWriteSummaryV1,
)
from observation_store import ObservationStore
from route_session_contract import (
    AnalysisSessionV1,
    CollectionSessionCloseV1,
    CollectionSessionV1,
    CollectionSourceMembershipCloseV1,
    CollectionSourceMembershipV1,
    RouteSessionProvenanceV1,
    RouteV1,
    create_analysis_session,
    create_collection_session,
    create_membership_close,
    create_route,
    create_session_close,
    create_source_membership,
    validate_collection_session_boundaries,
    validate_source_membership_boundaries,
    validate_source_record_admission,
)
from synthetic_jsonl_adapter import ReplaySummaryV1, replay_synthetic_jsonl
from synthetic_operator_fix_jsonl_adapter import (
    decode_synthetic_operator_fix_jsonl,
)

HMAC_KEY = b"dup-replay-idempotency-e2e-test-key"
SESSION_CONTROLLER_ID = "e2e_controller"
SESSION_REFERENCE = "dup_replay_001"
OBS_SENSOR_ID = "obs_sensor"
GPS_SENSOR_ID = "gps_sensor"
INGEST_TS_1 = 2_000_000
INGEST_TS_2 = 3_000_000
OBS_SOURCE_TS = 1_000_000
FIX_SOURCE_TS = 1_000_000
MAX_DELTA_US = 0

RS_PROV = RouteSessionProvenanceV1(
    controller_name="e2e_controller",
    controller_version="1.0.0",
    operation_mode="session_control",
)

AS_PROV = RouteSessionProvenanceV1(
    controller_name="e2e_controller",
    controller_version="1.0.0",
    operation_mode="analysis",
)

OBS_LINE = json.dumps(
    {"source_record_reference": "obs_001", "source_timestamp_us": OBS_SOURCE_TS},
    separators=(",", ":"),
    sort_keys=True,
)

FIX_LINE = json.dumps(
    {
        "source_record_reference": "fix_001",
        "operator_fix_timestamp_us": FIX_SOURCE_TS,
        "operator_latitude": 0,
        "operator_longitude": 0,
    },
    separators=(",", ":"),
    sort_keys=True,
)

_DIGEST_INPUT = json.dumps(
    [
        {
            "operation": "replay_synthetic_jsonl",
            "pass_index": 1,
            "records": [
                {
                    "source_record_reference": "obs_001",
                    "source_timestamp_us": OBS_SOURCE_TS,
                }
            ],
        },
        {
            "max_delta_us": MAX_DELTA_US,
            "operation": "run_bounded_observation_location_link_correlation",
            "operator_fixes": [
                {
                    "source_record_reference": "fix_001",
                    "operator_fix_timestamp_us": FIX_SOURCE_TS,
                    "operator_latitude": 0,
                    "operator_longitude": 0,
                }
            ],
            "pass_index": 1,
        },
        {
            "operation": "replay_synthetic_jsonl",
            "pass_index": 2,
            "records": [
                {
                    "source_record_reference": "obs_001",
                    "source_timestamp_us": OBS_SOURCE_TS,
                }
            ],
        },
        {
            "max_delta_us": MAX_DELTA_US,
            "operation": "run_bounded_observation_location_link_correlation",
            "operator_fixes": [
                {
                    "source_record_reference": "fix_001",
                    "operator_fix_timestamp_us": FIX_SOURCE_TS,
                    "operator_latitude": 0,
                    "operator_longitude": 0,
                }
            ],
            "pass_index": 2,
        },
    ],
    ensure_ascii=False,
    separators=(",", ":"),
    sort_keys=True,
)
INPUT_MANIFEST_DIGEST = hashlib.sha256(
    _DIGEST_INPUT.encode("utf-8")
).hexdigest()

EXPECTED_SUMMARY = GroundTruthExpectedSummaryV1(
    outcome="completed",
    rejection_stage=None,
    replay_summary=ReplaySummaryV1(
        total_records=1, inserted=0, duplicate=1, identity_conflict=0,
    ),
    location_link_write_summary=LocationLinkWriteSummaryV1(
        total_candidates=1, inserted=0, duplicate=1, identity_conflict=0,
    ),
    observation_event_count=1,
    observation_location_link_count=1,
    collection_session_count=1,
    source_membership_count=1,
    membership_close_count=1,
    session_close_count=1,
    route_count=1,
    analysis_session_count=1,
    unlinked_observation_count=0,
    route_point_counts=(1,),
    route_source_time_bounds_us=((OBS_SOURCE_TS, OBS_SOURCE_TS),),
    source_to_fix_deltas_us=(0,),
)

MANIFEST = GroundTruthScenarioManifestV1(
    schema_version=SCHEMA_VERSION_V1,
    record_kind=RECORD_KIND,
    scenario_id="cyt.test.duplicate_observation_replay_is_idempotent",
    scenario_version="1.0.0",
    scenario_label="Duplicate Replay Idempotency Scenario",
    tags=("duplicate-replay", "e2e", "idempotency"),
    input_manifest_digest=INPUT_MANIFEST_DIGEST,
    expected_summary=EXPECTED_SUMMARY,
)


class TestDuplicateReplayIdempotencyE2E(unittest.TestCase):
    def setUp(self):
        self.tempdir = tempfile.TemporaryDirectory()
        self.addCleanup(self.tempdir.cleanup)

    def _open_store(self):
        db_path = Path(self.tempdir.name) / "store.sqlite"
        store = ObservationStore(db_path)
        self.addCleanup(store.close)
        return store

    def test_duplicate_replay_idempotency_e2e(self):
        # --- pre-declared manifest checks ---
        self.assertEqual(MANIFEST.schema_version, "1.0")
        self.assertEqual(MANIFEST.record_kind, "ground_truth_scenario")
        self.assertRegex(
            MANIFEST.input_manifest_digest,
            r"^[0-9a-f]{64}$",
        )

        # --- create store ---
        store = self._open_store()

        # --- collection session ---
        session = create_collection_session(
            hmac_key=HMAC_KEY,
            session_controller_id=SESSION_CONTROLLER_ID,
            collection_session_reference=SESSION_REFERENCE,
            opened_source_timestamp_us=0,
            ingest_timestamp_us=INGEST_TS_1,
            provenance=RS_PROV,
        )

        # --- source membership ---
        membership = create_source_membership(
            hmac_key=HMAC_KEY,
            collection_session=session,
            source_type="synthetic.jsonl",
            sensor_id=OBS_SENSOR_ID,
            source_instance_reference="obs_instance_001",
            joined_source_timestamp_us=0,
            ingest_timestamp_us=INGEST_TS_1,
            provenance=RS_PROV,
        )

        # --- FIRST REPLAY: one observation (first pass, inserted) ---
        first_replay_summary = replay_synthetic_jsonl(
            [OBS_LINE],
            store=store,
            hmac_key=HMAC_KEY,
            collection_session_id=session.collection_session_id,
            sensor_id=OBS_SENSOR_ID,
            ingest_timestamp_us=INGEST_TS_1,
        )

        # --- decode one transient operator fix (never persisted) ---
        fixes = decode_synthetic_operator_fix_jsonl(
            [FIX_LINE],
            hmac_key=HMAC_KEY,
            collection_session_id=session.collection_session_id,
            sensor_id=GPS_SENSOR_ID,
            ingest_timestamp_us=INGEST_TS_1,
        )
        fix = fixes[0]

        # --- FIRST CORRELATION: bounded link write (first pass, inserted) ---
        first_link_summary = run_bounded_observation_location_link_correlation(
            store=store,
            hmac_key=HMAC_KEY,
            operator_fixes=fixes,
            max_delta_us=MAX_DELTA_US,
            collection_session_id=session.collection_session_id,
        )

        # --- assert first pass separately ---
        self.assertEqual(
            first_replay_summary,
            ReplaySummaryV1(
                total_records=1, inserted=1, duplicate=0, identity_conflict=0,
            ),
        )
        self.assertEqual(
            first_link_summary,
            LocationLinkWriteSummaryV1(
                total_candidates=1, inserted=1, duplicate=0, identity_conflict=0,
            ),
        )

        # --- capture state after first pass for idempotency assertion ---
        events_after_first = store.list_observation_events()
        links_after_first = store.list_observation_location_links()
        self.assertEqual(len(events_after_first), 1)
        self.assertEqual(len(links_after_first), 1)
        first_obs_id = events_after_first[0].observation_id
        first_link_id = links_after_first[0].location_link_id

        # --- SECOND REPLAY: same observation line, later ingest (duplicate) ---
        second_replay_summary = replay_synthetic_jsonl(
            [OBS_LINE],
            store=store,
            hmac_key=HMAC_KEY,
            collection_session_id=session.collection_session_id,
            sensor_id=OBS_SENSOR_ID,
            ingest_timestamp_us=INGEST_TS_2,
        )

        # --- SECOND CORRELATION: same fix objects, same bound (duplicate) ---
        second_link_summary = run_bounded_observation_location_link_correlation(
            store=store,
            hmac_key=HMAC_KEY,
            operator_fixes=fixes,
            max_delta_us=MAX_DELTA_US,
            collection_session_id=session.collection_session_id,
        )

        # --- close records (after observation/fix timestamp) ---
        membership_close = create_membership_close(
            hmac_key=HMAC_KEY,
            membership=membership,
            left_source_timestamp_us=2_000_000,
            close_reason="normal",
            ingest_timestamp_us=INGEST_TS_2,
            provenance=RS_PROV,
        )

        session_close = create_session_close(
            hmac_key=HMAC_KEY,
            collection_session=session,
            closed_source_timestamp_us=2_000_000,
            close_reason="completed",
            ingest_timestamp_us=INGEST_TS_2,
            provenance=RS_PROV,
        )

        # --- validate source-record admission ---
        validate_source_record_admission(
            session,
            membership,
            OBS_SOURCE_TS,
            session_close=session_close,
            membership_close=membership_close,
        )

        # --- validate lifecycle boundaries ---
        validate_collection_session_boundaries(
            session, session_close=session_close,
        )
        validate_source_membership_boundaries(
            membership,
            membership_close=membership_close,
            session_collection_session_id=session.collection_session_id,
        )

        # --- create route once from transient fix ---
        route = create_route(
            hmac_key=HMAC_KEY,
            collection_session_id=session.collection_session_id,
            operator_fixes=[fix],
            provenance=RouteSessionProvenanceV1(
                controller_name="e2e_controller",
                controller_version="1.0.0",
                operation_mode="route_snapshot",
            ),
            created_ingest_timestamp_us=INGEST_TS_2,
        )

        # --- create analysis session ---
        analysis_session = create_analysis_session(
            hmac_key=HMAC_KEY,
            analysis_type="ground_truth_review",
            analysis_version="1.0",
            collection_session_ids=[session.collection_session_id],
            route_ids=[route.route_id],
            input_manifest_digest=INPUT_MANIFEST_DIGEST,
            provenance=AS_PROV,
            created_ingest_timestamp_us=INGEST_TS_2,
        )

        # --- final persisted state ---
        events_final = store.list_observation_events()
        links_final = store.list_observation_location_links()

        # --- build actual summary using second-pass summaries ---
        actual_summary = build_completed_ground_truth_summary_v1(
            replay_summary=second_replay_summary,
            location_link_write_summary=second_link_summary,
            observations=events_final,
            location_links=links_final,
            collection_sessions=(session,),
            source_memberships=(membership,),
            membership_closes=(membership_close,),
            session_closes=(session_close,),
            routes=(route,),
            analysis_sessions=(analysis_session,),
        )

        # --- assert exact equality with manifest ---
        self.assertEqual(actual_summary, MANIFEST.expected_summary)

        # --- additional assertions ---
        # Final store: exactly one observation and one link
        self.assertEqual(len(events_final), 1)
        self.assertEqual(len(links_final), 1)
        self.assertEqual(events_final, events_after_first)
        self.assertEqual(links_final, links_after_first)

        # Stable IDs: second pass did not alter first persisted records
        self.assertEqual(events_final[0].observation_id, first_obs_id)
        self.assertEqual(links_final[0].location_link_id, first_link_id)

        # No identity_conflict in any pass
        self.assertEqual(first_replay_summary.identity_conflict, 0)
        self.assertEqual(first_link_summary.identity_conflict, 0)
        self.assertEqual(second_replay_summary.identity_conflict, 0)
        self.assertEqual(second_link_summary.identity_conflict, 0)

        obs = events_final[0]
        self.assertEqual(
            obs.collection_session_id,
            session.collection_session_id,
        )
        self.assertEqual(
            fix.collection_session_id,
            session.collection_session_id,
        )

        link = links_final[0]
        self.assertEqual(link.observation_id, obs.observation_id)

        self.assertEqual(route.started_source_timestamp_us, OBS_SOURCE_TS)
        self.assertEqual(route.ended_source_timestamp_us, OBS_SOURCE_TS)
        self.assertEqual(route.point_count, 1)

        self.assertEqual(
            analysis_session.ordered_collection_session_ids,
            (session.collection_session_id,),
        )
        self.assertEqual(
            analysis_session.ordered_route_ids,
            (route.route_id,),
        )

        self.assertRegex(
            MANIFEST.input_manifest_digest,
            r"^[0-9a-f]{64}$",
        )
