...
HMAC_KEY = b"equidistant-tie-break-e2e-test-key"
SESSION_CONTROLLER_ID = "e2e_controller"
SESSION_REFERENCE = "equidistant_tie_001"
OBS_SENSOR_ID = "obs_sensor"
GPS_SENSOR_ID = "gps_sensor"
INGEST_TS = 2_000_000
OBS_SOURCE_TS = 1_000_000
EARLIER_FIX_TS = 999_500
LATER_FIX_TS = 1_000_500
...
EARLIER_FIX_LINE = json.dumps(
    {
        "source_record_reference": "fix_earlier",
        "operator_fix_timestamp_us": EARLIER_FIX_TS,
        "operator_latitude": 0,
...
LATER_FIX_LINE = json.dumps(
    {
        "source_record_reference": "fix_later",
        "operator_fix_timestamp_us": LATER_FIX_TS,
        "operator_latitude": 0,
        "operator_longitude": 0,
    },
    separators=(",", ":"),
...
EXPECTED_SUMMARY = GroundTruthExpectedSummaryV1(
    ...
    route_source_time_bounds_us=((EARLIER_FIX_TS, LATER_FIX_TS),),
    source_to_fix_deltas_us=(-500,),
)

MANIFEST = GroundTruthScenarioManifestV1(
    schema_version=SCHEMA_VERSION_V1,
    record_kind=RECORD_KIND,
    scenario_id="cyt.test.equidistant_prefers_earlier_fix",
    scenario_version="1.0.0",
    scenario_label="Equidistant Candidates Prefer Earlier Fix Scenario",
    tags=("e2e", "tie-break"),
...
class TestEquidistantTieBreakPrefersEarlierFix(unittest.TestCase):
    def setUp(self):
    ...
    def test_equidistant_candidates_prefer_earlier_fix(self):
        ...
        # --- decode two operator fixes (later fix supplied first) ---
        fixes = decode_synthetic_operator_fix_jsonl(
            [LATER_FIX_LINE, EARLIER_FIX_LINE],
            hmac_key=HMAC_KEY,
            collection_session_id=session.collection_session_id,
            sensor_id=GPS_SENSOR_ID,
            ingest_timestamp_us=INGEST_TS,
        )
        fix_earlier, fix_later = fixes[1], fixes[0]
        ...
        # --- create route from both decoded fixes (later first order) ---
        route = create_route(
            hmac_key=HMAC_KEY,
            collection_session_id=session.collection_session_id,
            operator_fixes=[fix_later, fix_earlier],
            max_internal_gap_us=1_000,
        ...
        self.assertEqual(
            fix_earlier.collection_session_id,
            session.collection_session_id,
        )
        self.assertEqual(
            fix_later.collection_session_id,
            session.collection_session_id,
        )

        link = links[0]
        self.assertEqual(link.observation_id, obs.observation_id)
        self.assertEqual(link.operator_fix_id, fix_earlier.operator_fix_id)
        self.assertNotEqual(link.operator_fix_id, fix_later.operator_fix_id)
        self.assertEqual(link.operator_fix_timestamp_us, EARLIER_FIX_TS)
        self.assertEqual(link.source_to_fix_delta_us, -500)
        self.assertEqual(abs(link.source_to_fix_delta_us), CORRELATION_BOUND_US)

        self.assertEqual(route.started_source_timestamp_us, EARLIER_FIX_TS)
        self.assertEqual(route.ended_source_timestamp_us, LATER_FIX_TS)
        self.assertEqual(route.point_count, 2)
        self.assertEqual(
            route.ordered_operator_fix_ids,
            (fix_earlier.operator_fix_id, fix_later.operator_fix_id),
        )
